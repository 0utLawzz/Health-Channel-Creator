...
import { ThermometerSun, ShieldCheck, Droplets } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#2F6FED]/10 border border-[#2F6FED]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#2F6FED] font-display font-bold text-[22px] uppercase tracking-wider">What Pasteurisation Does</span>
        </motion.div>
        {[
          { icon: ThermometerSun, color: '#f97316', title: 'Kills dangerous bacteria', body: 'Salmonella, E. coli, Listeria — removed by heat treatment' },
          { icon: Droplets, color: '#2F6FED', title: 'Nutrients stay intact', body: 'Calcium and protein levels are essentially unchanged' },
          { icon: ShieldCheck, color: '#10b981', title: 'Safe for everyone', body: 'Including children, pregnant women, and immunocompromised individuals' },
        ].map((item, i) => {
