...
import { Timer, MoveRight, Dumbbell } from 'lucide-react';
  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        ...
        {[
          { icon: Timer, color: '#2F6FED', label: 'Move every 30-60 minutes', body: 'Stand, stretch, or walk briefly — reset the stiffness clock' },
          { icon: MoveRight, color: '#10b981', label: 'Vary your positions', body: 'Sit, stand, recline — no single position should dominate your day' },
          { icon: Dumbbell, color: '#f97316', label: 'Strengthen supporting muscles', body: 'Core and back strength helps more than posture policing' },
        ].map((item, i) => {
