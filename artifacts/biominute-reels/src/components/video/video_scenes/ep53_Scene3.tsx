    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">Raw Milk Risk Is Real</span>
        </motion.div>
        {[
          { icon: AlertTriangle, color: '#f97316', label: 'Salmonella, E. coli, Listeria', body: 'All documented in raw milk outbreaks — especially risk for kids' },
          { icon: ShieldCheck, color: '#10b981', label: 'Pasteurisation removes all of this', body: 'Without touching the nutritional profile you care about' },
          { icon: CheckCircle2, color: '#2F6FED', label: 'Same calcium. Same protein.', body: 'Make the safer choice — the nutrient upside simply isn\'t there' },
        ].map((item, i) => {
