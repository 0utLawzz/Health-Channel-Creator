...
import { Dna, Pill, CheckCircle2 } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">What to Actually Watch For</span>
        </motion.div>
        {[
          { icon: Dna, color: '#f97316', label: 'Family history of hair loss?', body: 'Talk to a dermatologist — early intervention helps most' },
          { icon: Pill, color: '#2F6FED', label: 'Nutritional deficiencies', body: 'Iron, zinc, biotin — worth checking with a blood test' },
          { icon: CheckCircle2, color: '#10b981', label: 'Wash freely', body: 'Daily washing is safe — use a gentle shampoo that suits your scalp' },
        ].map((item, i) => {
