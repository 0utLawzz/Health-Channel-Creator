    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">Food First Approach</span>
        </motion.div>
        {[
          { icon: Leaf, color: '#10b981', label: 'Leafy greens, nuts, whole grains', body: 'Common foods cover most people\'s magnesium needs' },
          { icon: CheckCircle2, color: '#2F6FED', label: 'Supplement if truly deficient', body: 'Worth testing levels before spending on capsules' },
          { icon: AlertCircle, color: '#f97316', label: 'Not a sleep pill for everyone', body: 'Manage expectations — impact varies by individual status' },
        ].map((item, i) => {
