...
import { FlaskConical, Leaf, CheckCircle2 } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">What Research Actually Shows</span>
        </motion.div>
        {[
          { icon: FlaskConical, color: '#2F6FED', title: 'Not toxic in normal amounts', body: 'Canola, sunflower, soybean — safe at typical cooking doses' },
          { icon: Leaf, color: '#10b981', title: 'Some cardiovascular benefit', body: 'Replacing saturated fats shows positive outcomes in studies' },
          { icon: CheckCircle2, color: '#f97316', title: 'Inflammation claim not supported', body: 'Research doesn\'t show widespread inflammation from these oils' },
        ].map((item, i) => {
