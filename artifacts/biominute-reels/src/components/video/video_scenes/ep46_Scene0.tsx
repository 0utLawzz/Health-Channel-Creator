...
import { Flame, Activity, Scale } from 'lucide-react';
...
export function Scene0() {
  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <motion.div className="absolute top-[12%] w-[520px] h-[520px] bg-gradient-to-tr from-[#10b981]/15 to-[#f97316]/10 rounded-full blur-[130px]"
      ...
      <div className="absolute top-[185px] flex flex-col items-center gap-6 z-10 w-full px-12">
        <motion.div className="relative w-[220px] h-[220px]"
          ...
          {/* Flame (fasted) */}
          <motion.div className="absolute top-0 left-0" animate={{ y: [0, -8, 0] }} transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}>
            <div className="relative"><div className="absolute inset-0 bg-[#f97316]/20 blur-[20px] rounded-full" />
              <Flame size={80} color="#f97316" strokeWidth={1.3} /></div>
          </motion.div>
          {/* Activity (running) */}
          <motion.div className="absolute bottom-4 right-0" animate={{ x: [0, 6, 0] }} transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut' }}>
            <Activity size={72} color="#10b981" strokeWidth={1.4} />
          </motion.div>
          {/* Scale */}
          <motion.div className="absolute top-14 right-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0 }}>
        ...
        <motion.div className="flex items-center gap-3 bg-[#10b981]/10 border border-[#10b981]/30 px-7 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[18px] uppercase tracking-wider">S2 • Movement &amp; Body</span>
        </motion.div>
      </div>
