...
import { Scale, Target, CheckCircle2 } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[150px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[22px] uppercase tracking-wider">What Actually Drives Results</span>
        </motion.div>
        {[
          { icon: Target, color: '#f97316', label: 'Hit your daily protein target', body: 'Amount matters more than animal vs plant source' },
          { icon: Scale, color: '#2F6FED', label: 'Combine plant sources', body: 'Beans + rice, lentils + bread — all amino acids covered' },
          { icon: CheckCircle2, color: '#10b981', label: 'Both diets build muscle equally', body: 'Studies show similar hypertrophy when protein intake is matched' },
        ].map((item, i) => {
