  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[230px] z-10 w-[90%]">
        <motion.div className="bg-[#1e293b]/90 border border-[#334155] rounded-[40px] p-12 flex flex-col items-center text-center shadow-2xl relative overflow-hidden"
          ...
          <motion.div initial={{ scale: 0, rotate: -45 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: 0.6, ...SPRING_SNAPPY }}>
            <div className="w-24 h-24 rounded-full bg-[#10b981] flex items-center justify-center">
              <Heart size={48} color="#0F172A" fill="#0F172A" strokeWidth={2} />
            </div>
          </motion.div>
          <motion.h2 className="text-[#f8fafc] text-[52px] font-bold uppercase tracking-wider font-display leading-tight mt-8"
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
            How Do You
            <span className="text-[#f97316] block mt-2">Like Your</span>
            <span className="text-[#10b981] block mt-2">Eggs?</span>
          </motion.h2>
          <motion.div className="flex items-center gap-3 mt-8 bg-[#0F172A]/80 px-6 py-4 rounded-xl border border-white/10"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 1 }}>
            <MessageCircle size={28} color="#14b8a6" />
            <span className="text-[#94a3b8] text-[24px] font-medium">Scrambled? Poached? 👇</span>
          </motion.div>
        </motion.div>
      </div>
      <motion.div className="absolute w-full px-12 z-30 flex justify-center" style={{ bottom: BOTTOM_SAFE_ZONE_PX + 60 }}
        ...
        <div className="flex items-center gap-4 text-[#94a3b8] text-[22px] font-medium bg-[#0F172A]/90 px-6 py-4 rounded-xl backdrop-blur-sm border border-white/10 shadow-lg">
          ...
          <p className="leading-relaxed text-left">Source: General nutrition research on dietary cholesterol and cardiovascular risk in healthy populations</p>
        </div>
      </motion.div>
    </motion.div>
  );
