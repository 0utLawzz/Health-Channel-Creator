    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[190px] flex flex-col items-center gap-7 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[24px] uppercase tracking-wider">Smarter Tracking Habits</span>
        </motion.div>

        {[
          { icon: Calendar, color: '#2F6FED', title: 'Same Time Each Day', body: 'Morning, after bathroom, before eating' },
          { icon: Scale, color: '#f97316', title: 'Weekly Average', body: 'Add 7 readings, divide by 7' },
          { icon: TrendingDown, color: '#10b981', title: 'Look at the Trend', body: 'Downward over weeks = fat loss progress' },
        ].map((item, i) => {
      ...
      <div className="absolute w-full px-12 text-center z-20" style={{ bottom: BOTTOM_SAFE_ZONE_PX + 80 }}>
        <motion.h2 className="text-[#f8fafc] text-[56px] font-bold uppercase tracking-wider font-display leading-tight"
          ...
          Weigh Weekly,
          <motion.span className="text-[#10b981] block mt-2 drop-shadow-md" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1.6, ...SPRING_SNAPPY }}>Not Daily</motion.span>
        </motion.h2>
      </div>
    </motion.div>
  );
