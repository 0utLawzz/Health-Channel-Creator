...
import { Scale, Dumbbell, Leaf } from 'lucide-react';
...
export function Scene0() {
  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <motion.div className="absolute top-[12%] w-[520px] h-[520px] bg-gradient-to-tr from-[#10b981]/15 to-[#2F6FED]/10 rounded-full blur-[130px]"
      ...
      <div className="absolute top-[185px] flex flex-col items-center gap-6 z-10 w-full px-12">
        <motion.div className="relative w-[220px] h-[220px]"
          ...
          <motion.div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
            ...
            <div className="relative"><div className="absolute inset-0 bg-[#10b981]/20 blur-[30px] rounded-full" />
              <Scale size={96} color="#10b981" strokeWidth={1.3} /></div>
          </motion.div>
          <motion.div className="absolute top-0 right-0" animate={{ opacity: [0.7, 1, 0.7] }} transition={{ duration: 2, repeat: Infinity }}>
            <Dumbbell size={52} color="#f97316" strokeWidth={1.5} />
          </motion.div>
          <motion.div className="absolute bottom-0 left-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}>
            <Leaf size={44} color="#2F6FED" strokeWidth={1.5} />
          </motion.div>
        </motion.div>
