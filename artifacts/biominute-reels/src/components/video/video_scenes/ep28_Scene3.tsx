...
import { Gauge, Dumbbell, CheckCircle } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[190px] flex flex-col items-center gap-7 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[24px] uppercase tracking-wider">The Truth</span>
        </motion.div>

        {[
          { icon: Gauge, color: '#14b8a6', title: 'Not Broken', body: 'Metabolism adapts — it never stops working' },
          { icon: Dumbbell, color: '#2F6FED', title: 'Build & Lift', body: 'Muscle is your highest-burn tissue' },
          { icon: CheckCircle, color: '#10b981', title: 'Consistency Wins', body: 'Small daily actions over quick fixes' },
        ].map((item, i) => {
      ...
      <div className="absolute w-full px-12 text-center z-20" style={{ bottom: BOTTOM_SAFE_ZONE_PX + 80 }}>
        <motion.h2 className="text-[#f8fafc] text-[56px] font-bold uppercase tracking-wider font-display leading-tight"
          ...
          Your Metabolism
          <motion.span className="text-[#14b8a6] block mt-2 drop-shadow-md" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1.6, ...SPRING_SNAPPY }}>Is Not Broken</motion.span>
        </motion.h2>
      </div>
    </motion.div>
  );
