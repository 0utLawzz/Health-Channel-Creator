...
import { Heart, ShieldCheck, Salad } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[190px] flex flex-col items-center gap-7 z-10 w-full px-10">
        <motion.div className="bg-[#10b981]/10 border border-[#10b981]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#10b981] font-display font-bold text-[24px] uppercase tracking-wider">The Bottom Line</span>
        </motion.div>
        {[
          { icon: ShieldCheck, color: '#10b981', title: 'Eggs in Moderation', body: 'Not linked to heart disease in most healthy adults' },
          { icon: Salad, color: '#2F6FED', title: 'Focus on Diet Quality', body: 'Whole foods, veggies, and variety matter most' },
          { icon: Heart, color: '#f97316', title: 'No Single Food Rules', body: 'Context and patterns beat fixating on one food' },
        ].map((item, i) => {
          const Icon = item.icon;
          return (
            <motion.div key={i} className="bg-[#1e293b] border border-[#334155] rounded-[28px] px-8 py-6 flex items-center gap-6 w-full"
              ...
              <div>
                <p className="text-[#f8fafc] font-display font-bold text-[24px] uppercase leading-tight">{item.title}</p>
      ...
      <div className="absolute w-full px-12 text-center z-20" style={{ bottom: BOTTOM_SAFE_ZONE_PX + 80 }}>
        <motion.h2 className="text-[#f8fafc] text-[56px] font-bold uppercase tracking-wider font-display leading-tight"
          ...
          No Single Food
          <motion.span className="text-[#10b981] block mt-2 drop-shadow-md" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1.6, ...SPRING_SNAPPY }}>Makes or Breaks Health</motion.span>
        </motion.h2>
      </div>
    </motion.div>
  );
