...
import { FlaskConical, Flame, Leaf } from 'lucide-react';
      ...
      <div className="absolute top-[185px] flex flex-col items-center gap-6 z-10 w-full px-12">
        <motion.div className="relative w-[220px] h-[220px]"
          ...
          <motion.div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className="relative"><div className="absolute inset-0 bg-[#f97316]/20 blur-[30px] rounded-full" />
              <FlaskConical size={96} color="#f97316" strokeWidth={1.3} /></div>
          </motion.div>
          <motion.div className="absolute top-0 right-0" animate={{ opacity: [0.6, 1, 0.6], scale: [1, 1.15, 1] }} transition={{ duration: 1.8, repeat: Infinity }}>
            <Flame size={52} color="#ef4444" strokeWidth={1.5} />
          </motion.div>
          <motion.div className="absolute bottom-0 left-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}>
            <Leaf size={44} color="#10b981" strokeWidth={1.5} />
          </motion.div>
        </motion.div>
