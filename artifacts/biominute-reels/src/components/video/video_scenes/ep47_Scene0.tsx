...
import { User, AlertCircle, MoveRight } from 'lucide-react';
...
export function Scene0() {
  ...
  return (
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <motion.div className="absolute top-[12%] w-[520px] h-[520px] bg-gradient-to-tr from-[#f97316]/15 to-[#2F6FED]/10 rounded-full blur-[130px]"
      ...
      <div className="absolute top-[185px] flex flex-col items-center gap-6 z-10 w-full px-12">
        <motion.div className="relative w-[220px] h-[220px]"
          ...
          {/* Stiff upright figure */}
          <motion.div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <User size={100} color="#2F6FED" strokeWidth={1.3} />
          </motion.div>
          {/* Stiffness alert icons building up */}
          {[0, 1, 2].map(i => (
            <motion.div key={i} className="absolute" style={{ top: `${15 + i * 22}%`, right: '5%' }}
              initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.8 + i * 0.5, ...SPRING_SNAPPY }}>
              <AlertCircle size={30} color="#f97316" strokeWidth={1.8} />
            </motion.div>
          ))}
          <motion.div className="absolute bottom-0 right-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }}>
            <MoveRight size={44} color="#10b981" strokeWidth={1.5} />
          </motion.div>
        </motion.div>
