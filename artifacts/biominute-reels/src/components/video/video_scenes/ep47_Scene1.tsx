...
import { AlertCircle, MoveRight, Clock } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[160px] flex flex-col items-center gap-5 z-10 w-full px-10">
        <motion.div className="bg-[#f97316]/10 border border-[#f97316]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#f97316] font-display font-bold text-[22px] uppercase tracking-wider">The Real Issue</span>
        </motion.div>
        {[
          { icon: AlertCircle, color: '#f97316', title: 'No perfect posture exists', body: 'Even "ideal" posture causes stiffness when held for hours' },
          { icon: Clock, color: '#2F6FED', title: 'Static positions = the problem', body: 'It\'s staying still too long, not posture angle, that causes pain' },
          { icon: MoveRight, color: '#10b981', title: 'Movement breaks help most', body: 'Regular position shifts keep muscles and joints healthy' },
        ].map((item, i) => {
