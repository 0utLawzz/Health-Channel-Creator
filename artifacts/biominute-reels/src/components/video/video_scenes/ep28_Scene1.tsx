...
import { User, Dumbbell, Moon, TrendingDown } from 'lucide-react';
    ...
    <motion.div className="absolute inset-0 w-full h-full bg-[#0F172A] flex flex-col items-center justify-center overflow-hidden font-body"
      ...
      <div className="absolute top-[185px] flex flex-col items-center gap-6 z-10 w-full px-10">
        <motion.div className="bg-[#14b8a6]/10 border border-[#14b8a6]/30 px-8 py-4 rounded-2xl"
          ...
          <span className="text-[#14b8a6] font-display font-bold text-[24px] uppercase tracking-wider">Metabolism Adapts, Not Breaks</span>
        </motion.div>

        {[
          { icon: User, color: '#f97316', title: 'Age', body: 'Slows ~1–2% per decade after 30' },
          { icon: Dumbbell, color: '#2F6FED', title: 'Muscle Mass', body: 'More muscle = higher resting burn' },
          { icon: Moon, color: '#14b8a6', title: 'Hormones & Sleep', body: 'Cortisol and poor sleep slow it down' },
          { icon: TrendingDown, color: '#10b981', title: 'Calorie Restriction', body: 'Body adapts to eat less — not broken' },
        ].map((item, i) => {
          const Icon = item.icon;
          return (
            <motion.div key={i} className="bg-[#1e293b] border border-[#334155] rounded-[24px] px-7 py-5 flex items-center gap-5 w-full"
              initial={{ x: -40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 + i * 0.25, ...SPRING_SMOOTH }}>
              ...
              <div>
                <p className="font-display font-bold text-[22px] uppercase" style={{ color: item.color }}>{item.title}</p>
                <p className="text-[#94a3b8] text-[19px] mt-0.5">{item.body}</p>
              </div>
            </motion.div>
          );
        })}
      ...
      <div className="absolute w-full px-12 text-center z-20" style={{ bottom: BOTTOM_SAFE_ZONE_PX + 80 }}>
        <motion.h2 className="text-[#f8fafc] text-[54px] font-bold uppercase tracking-wider font-display leading-tight"
          initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }}>
          Metabolism
          <motion.span className="text-[#14b8a6] block mt-2 drop-shadow-md" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1.6, ...SPRING_SNAPPY }}>Adapts</motion.span>
        </motion.h2>
      </div>
    </motion.div>
  );
